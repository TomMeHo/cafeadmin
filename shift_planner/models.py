...
class Shift(models.Model):
    ...
    # registered = models.ManyToManyField(User, through="ShiftRegistration")
    ...
    def __str__(self):
        dFormat = '%d.%m.%y'
        tFormat = '%H:%M'
        return f"{self.date.strftime(dFormat)}, von {self.starts_at.strftime(tFormat)} bis {self.ends_at.strftime(tFormat)}"

    @property
    def unregister_possible(self):
        last_unregister_date = self.date - timedelta(days = UNREGISTER_MIN_BEFORE_DAYS)
    ...
    @property
    def is_past(self):
        start_date = datetime.combine(self.date, self.starts_at)
    ...
    def is_registered(self, user: User):
    ...
    @staticmethod
    def get_shifts_by_user(user: User):
        return Shift.objects.filter(registree=user)
    
    @staticmethod
    def get_shifts_for_range(date_from: date, date_to: date):
        shifts = Shift.objects.filter(date__gte = date_from, date__lte = date_to)
        holidays = Holiday.objects.filter(date__gte = date_from, date__lte = date_to)
        days = []
        delta = timedelta( days = 1 )

        while( date_from <= date_to ):
            holiday = holidays.filter(date=date_from)
            shifts_for_date = shifts.filter(date = date_from).order_by('starts_at')
...
class ShiftRegistration(models.Model):
    ...
    registree = models.ForeignKey(User, related_name='user_registree', on_delete=models.CASCADE, default='')
    registered_at = models.DateTimeField(null = True, blank = True)
    registered_by = models.ForeignKey(User, related_name='user_registered_by', on_delete=models.DO_NOTHING, null = True, blank = True)
...
class CalendarDay:
    ...
    @staticmethod
    def getSpecificDay(specific_date : date, user : User) -> Self:
        shifts = Shift.objects.filter(date = specific_date).order_by('starts_at')
        for shift in shifts:
...
class CalendarWeek:
    ...
    @staticmethod
    def getSpecificWeek(any_day_of_week : date, user : User) -> Self:
...
class CalendarMonth:
    ...
    @staticmethod
    def getMonth(month : int, year : int, user : User):
